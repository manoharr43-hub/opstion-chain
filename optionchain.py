...
# FIXED VERSION — Token Flow Complete గా Fix చేయబడింది
# =========================================================
...
# =========================================================
# SHOONYA API CLASS — COMPLETE FIX
# =========================================================

class ShoonyaAPI:
    """
    Shoonya (Finvasia) REST API — Token Flow Complete గా Fix చేయబడింది

    FIX చేసినవి:
    1. get_scrip_token()  — symbol నుండి numeric token తీసుకోవడం
    2. token_cache        — ప్రతిసారి API call చేయకుండా cache చేయడం
    3. get_quote()        — token తో correct గా call చేయడం
    4. get_live_ltp_bulk()— bulk fetch లో token use చేయడం
    5. Error handling     — అన్ని చోట్లా proper errors చూపించడం
    """
    ...
    def __init__(self):
        ...
        # ✅ FIX 1: Token Cache — ప్రతిసారి API call చేయకూడదు
        # key: "NSE:RELIANCE" → value: "2885"
        self.token_cache = {}
    ...
    def login(self, user_id, password, totp, vendor_code, api_secret, imei="abc1234"):
        """
        Shoonya Login
        password SHA256 hash చేసి పంపాలి
        """
        try:
    ...
    def _post(self, endpoint, jdata):
        """Internal POST — session token automatically attach చేస్తుంది"""
        try:
            ...
            # ✅ FIX 2: Error దాచిపెట్టకుండా return చేయడం
            return {"stat": "Not_Ok", "emsg": str(e)}

    # ----------------------------------------------------------
    # ✅ NEW: GET SCRIP TOKEN — ఇది కొత్తగా add చేయబడింది
    # ----------------------------------------------------------

    def get_scrip_token(self, exchange, symbol):
        """
        Symbol పేరు నుండి Numeric Token తీసుకోవడం

        ఎందుకు కావాలి?
        - Shoonya API కి "RELIANCE" అర్థం కాదు
        - దానికి "2885" లాంటి numeric token కావాలి
        - SearchScrip endpoint తో token వస్తుంది
        ...
        Returns:
            token string లేదా None
        """
        if not self.session_token:
            return None

        # Cache లో ఉంటే direct గా return చేయడం — API call వద్దు
        cache_key = f"{exchange}:{symbol}"
        if cache_key in self.token_cache:
            return self.token_cache[cache_key]

        try:
            data = self._post("SearchScrip", {
                "uid": self.user_id,
                "stext": symbol,
                "exch": exchange
            })
            ...
            # Exact match వెతకడం — "RELIANCE-EQ" కాదు, "RELIANCE" కావాలి
            for item in values:
                tsym = item.get("tsym", "")
                # NSE equity లో symbol-EQ format ఉంటుంది
                # Example: "RELIANCE-EQ" → strip చేసి compare చేయాలి
                clean_tsym = tsym.replace("-EQ", "").replace("-BE", "")
                if clean_tsym == symbol:
                    token = item.get("token")
                    # Cache లో save చేయడం
                    self.token_cache[cache_key] = token
                    return token

            # Exact match రాకపోతే first result తీసుకోవడం
            if values:
            ...
            st.warning(f"SearchScrip error ({symbol}): {e}")
            return None

    # ----------------------------------------------------------
    # ✅ FIXED: GET QUOTE — Token తో పని చేస్తుంది
    # ----------------------------------------------------------

    def get_quote(self, exchange, symbol):
        """
        Live Quote తీసుకోవడం

        FIX: మొదట token తీసుకుని, తర్వాత GetQuotes call చేయడం

        Args:
            exchange: "NSE"
            symbol:   "RELIANCE" (symbol పేరు, token కాదు)
        """
        if not self.session_token:
            return None

        # ✅ మొదట token తీసుకోవడం
        token = self.get_scrip_token(exchange, symbol)

        if not token:
            st.warning(f"⚠️ Token దొరకలేదు: {symbol}")
            return None

        # ✅ Token తో GetQuotes call చేయడం
        data = self._post("GetQuotes", {
            "uid": self.user_id,
            "exch": exchange,
            "token": token          # ← ఇక్కడ token పంపడం ముఖ్యం
        })

        if data.get("stat") == "Ok":
            return data
        else:
            st.warning(f"⚠️ GetQuotes failed ({symbol}): {data.get('emsg', 'Unknown')}")
            return None
    ...
    def get_option_chain(self, symbol, expiry, strike_count=10):
        """
        Option Chain data తీసుకోవడం
        symbol: NIFTY or BANKNIFTY
        expiry: format — 24-Oct-2024
        """
        if not self.session_token:
            return pd.DataFrame()
        ...
        try:
            ...
            if data.get("stat") != "Ok":
                st.warning(f"Option Chain error: {data.get('emsg', 'Unknown')}")
                return pd.DataFrame()
            ...
            st.error(f"Option Chain exception: {e}")
            return pd.DataFrame()

    # ----------------------------------------------------------
    # ✅ FIXED: BULK LTP FETCH
    # ----------------------------------------------------------
    ...
    def get_live_ltp_bulk(self, symbols_list):
        ...
        FIX: మొదట tokens fetch చేసి, తర్వాత quotes తీసుకోవడం

        Args:
            symbols_list: [("NSE", "RELIANCE"), ("NSE", "TCS"), ...]

        Returns:
            dict: {"RELIANCE": {"ltp": 2500.0, "change_pct": 1.2, ...}}
        """
        if not self.session_token:
            return {}
        ...
        for exch, sym in symbols_list:
            try:
                # ✅ FIX: get_quote లో token logic ఉంది, direct గా call చేయడం
                ...
                # Rate limit avoid — 100ms wait
                time.sleep(0.1)

            except Exception as e:
                # ✅ FIX: error దాచిపెట్టకుండా log చేయడం
                st.warning(f"⚠️ {sym} fetch error: {e}")
    ...
    def get_expiry_list(self, symbol):
        ...
        st.warning(f"Expiry list error: {data.get('emsg', 'Unknown')}")
        return []

    # ----------------------------------------------------------
    # ✅ NEW: TOKEN CACHE VIEWER (Debug కోసం)
    # ----------------------------------------------------------

    def show_token_cache(self):
        """Cache లో ఉన్న tokens చూడడానికి — debugging కోసం"""
        if self.token_cache:
            return pd.DataFrame(
                list(self.token_cache.items()),
...
# ✅ FIX: Scanner results session లో save చేయడం — rerun లో మళ్ళీ scan వద్దు
...
if not st.session_state.shoonya_logged_in:
    with st.sidebar.expander("🔑 Login Credentials", expanded=True):
        ...
        st.caption("Credentials: [shoonya.com](https://shoonya.com)")

else:
    st.sidebar.success("✅ Shoonya Connected")

    # ✅ NEW: Token cache status చూపించడం
    cache_df = st.session_state.shoonya.show_token_cache()
    if not cache_df.empty:
        st.sidebar.caption(f"🗃️ Token Cache: {len(cache_df)} symbols loaded")
...
def generate_signal(latest):
    ...
    if latest["SMART_MONEY"]:               score += 10
    ...
    if st.session_state.shoonya_logged_in:
        ...
        if expiry_list:
            ...
            if st.button("🔄 LOAD OPTION CHAIN"):
                with st.spinner("Option Chain load అవుతోంది..."):
                ...
                if not option_df.empty:
                    ...
                    if pcr > 1.15:   st.success(f"🚀 BULLISH | PCR: {round(pcr, 2)}")
                    elif pcr < 0.85: st.error(f"🔻 BEARISH | PCR: {round(pcr, 2)}")
            ...
            st.warning("⚠️ Expiry dates రాలేదు.")
...
# =========================================================
# TAB 3 — AI SCANNER
# ✅ FIX: Button తో మాత్రమే scan చేయడం — auto scan వద్దు
# =========================================================
...
with tab3:
    ...
    # ✅ FIX: Button పెట్టడం — auto scan వద్దు
    col_run, col_clear = st.columns([3, 1])
    run_scan   = col_run.button("▶️ SCANNER RUN చేయండి", type="primary")
    clear_scan = col_clear.button("🗑️ Clear")
    ...
    if run_scan:
        ...
        if use_shoonya:
            with st.spinner("🔄 Shoonya నుండి live prices fetch చేస్తున్నాం..."):
                syms = [("NSE", v) for v in shoonya_symbols.values()]
        ...
        def scan_stock(item):
            ...
            try:
                ...
                return {
                    ...
                    "SCORE":      score,
                    # ✅ FIX: bool() cast
                    "SMART_MONEY": bool(latest["SMART_MONEY"]),
                }
            except Exception as e:
                st.warning(f"⚠️ {name} scan error: {e}")
                return None
    ...
    # Results చూపించడం — session లో ఉన్నవి
    if st.session_state.scan_results:
        ...
        st.info("👆 పైన 'SCANNER RUN చేయండి' button నొక్కండి")
...
st.markdown("---")
st.caption("🚀 NSE AI PRO MAX V9.0 ULTRA + Shoonya Live Integration — Fixed Version")
