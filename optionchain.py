...
st.markdown("**Auto-Memory Clear | Clean Excel Export | Signal Time Tracker | Advanced SMC & CISD | XGBoost AI**")
st.markdown("---")
...
if 'v11_master_data' not in st.session_state:
    ...
    # ఆటో-మెమరీ క్లీనర్ (పాత టేబుల్స్ ఉంటే ఎర్రర్ రాకుండా దానంతటదే క్లీన్ చేస్తుంది)
...
# ఎక్సెల్ ఫైల్ క్లీన్ చేయడానికి ఫంక్షన్
...
def process_stock_thread(symbol, interval, period, h52w, l52w, nifty_return, daily_close_series):
    ...
    brk_sig = "NO"
    if close > breakout_high: brk_sig, _ = "BULLISH", alerts.append("📈 Breakout
